...
def is_encoded(text):
    ...
    # Detect Base32 specifically
    if re.fullmatch(base32_pattern, text) and "1" not in text and "8" not in text:
        return "Base32"
    # Detect Base64 with characteristic checks
        ...
        if is_multiple_of_4 and ends_with_equals and alpha_num_ratio > 0.7:
            return "Base64"
    # Detect Hex encoding
    elif re.fullmatch(hex_pattern, text) and len(text.strip()) % 2 == 0:
        return "Hex"
    # Detect Base85 encoding
    elif re.fullmatch(base85_pattern, text) and len(text) % 5 == 0:
        return "Base85"
    # Detect URL Encoding
    elif re.fullmatch(url_pattern, text):
        return "URL Encoding"
    # Detect UUencode
    elif re.search(uuencode_pattern, text):
        return "UUencode"
