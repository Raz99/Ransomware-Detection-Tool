...
def create_base64_file():
    ...
    save_to_file("example/base64_encoded.txt", encoded)

# 2. Hex Encoding
def create_hex_file():
    encoded = text.encode().hex().encode()
    save_to_file("example/hex_encoded.txt", encoded)

# 3. Base32 Encoding
def create_base32_file():
    encoded = base64.b32encode(text.encode())
    save_to_file("example/base32_encoded.txt", encoded)

# 4. URL Encoding
def create_url_encoded_file():
    encoded = urllib.parse.quote(text).encode()
    save_to_file("example/url_encoded.txt", encoded)
...
def create_uuencode_file():
    ...
    save_to_file("example/uu_encoded.txt", encoded)

# 6. Quoted-Printable Encoding
def create_quoted_printable_file():
    encoded = quopri.encodestring(text.encode())
    save_to_file("example/quoted_printable.txt", encoded)

# 7. Base85 Encoding
def create_base85_file():
    encoded = base64.b85encode(text.encode())
    save_to_file("example/base85_encoded.txt", encoded)

# 8. Rot13 Encoding
def create_rot13_file():
    encoded = codecs.encode(text, "rot_13")
    save_to_file("example/rot13_encoded.txt", encoded.encode(), mode="wb")
